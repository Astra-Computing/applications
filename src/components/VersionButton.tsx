...
const HISTORY = [
  { v: '0.6.0', note: 'The host can now run the game hands-off: tick Advance automatically and each round resolves and moves on by itself, even if everyone has wandered off. Tap the QR code to fill the screen with it so people across the room can scan. Tap a player name to remove someone who has left - their votes still count, and they can rejoin. The quote that draws a BYE is now picked at random each round, and never twice running. Names with unusual characters or spacing are handled properly at join.' },
